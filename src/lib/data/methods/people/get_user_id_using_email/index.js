...
const getUserIdUsingEmail = function (email) {
  return new Promise(((resolve, reject) => {
    ...
    // models.sequelize.query(`SELECT people_id FROM people_information as P WHERE (SELECT JSON_CONTAINS((SELECT data FROM people_information WHERE people_id = P.id AND slug_id = (SELECT id FROM people_information_slugs WHERE slug_name = "email")), '"${email}"') as check_flag) = 1`,
